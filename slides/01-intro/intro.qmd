...
$$
where $\mathbf{z}^{(t)}$ are pseudo-responses and $\mathbf{W}^{(t)}$ is a diagonal matrix with nonnegative weights on the diagonal. Superscript $t$ is the iterate number.

Question: How much speedup we can achieve, by careful consideration of flops and memory usage, over the naive implementation?
```julia
